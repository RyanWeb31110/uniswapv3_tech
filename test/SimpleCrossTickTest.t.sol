...
/// @title 简单跨Tick交换测试
/// @notice 基于现有工作测试的跨Tick交换验证
contract SimpleCrossTickTest is Test {
    // ============ 测试状态变量 ============
    ...
    function testCrossTickSwap() public {
        // 设置第一个价格区间：85176 - 86129 (5000 - 5500)
        int24 lowerTick1 = 85176;
        int24 upperTick1 = 86129;
        uint128 liquidity1 = 1000000000000000000; // 使用更小的流动性数量

        // 设置第二个价格区间：86129 - 87173 (5500 - 6105)
        int24 lowerTick2 = 86129;
        int24 upperTick2 = 87173;
        uint128 liquidity2 = 1000000000000000000; // 使用更小的流动性数量
        ...
        // 添加第一个价格区间的流动性
        pool.mint(address(this), lowerTick1, upperTick1, liquidity1, data);

        // 添加第二个价格区间的流动性
        pool.mint(address(this), lowerTick2, upperTick2, liquidity2, data);
        ...
        // 验证流动性发生了变化（从第一个区间切换到第二个区间）
        assertTrue(liquidityAfter != liquidityBefore, "Liquidity should have changed");
    }
